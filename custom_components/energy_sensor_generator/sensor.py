...
class EnergySensor(SensorEntity, RestoreEntity):
	...
	def __init__(self, hass, base_name, source_sensor, storage_path, device_identifiers=None, friendly_name_override=None, constant_config=None):
		...
		self._fallback_store = None  # Lazy-init fallback if StorageManager not available
	...
	async def _load_state(self):
		...
		if self._storage_manager:
			...
			if not self._fallback_store:
				...
				self._fallback_store = ha_storage.Store(self._hass, version=1, key="energy_sensor_generator")
			storage = await self._fallback_store.async_load() or {}
		...
		try:
			if self._storage_manager:
				...
				if not self._fallback_store:
					from homeassistant.helpers import storage as ha_storage
					self._fallback_store = ha_storage.Store(self._hass, version=1, key="energy_sensor_generator")
				await self._fallback_store.async_save(storage)
		except Exception:
			pass
...
class DailyEnergySensor(SensorEntity, RestoreEntity):
	...
	def __init__(self, hass, base_name, source_sensor, storage_path, device_identifiers=None):
		...
		self._state = 0.0
		self._last_energy = None  # None = not yet initialised; 0.0 is a valid energy reading
	...
	async def _load_state(self):
		...
		self._last_energy = state_data.get("last_energy", None)
	...
	async def _handle_midnight_reset(self, now):
		...
		if state and state.state not in ("unknown", "unavailable"):
			try:
				...
				self._last_energy = None  # Will re-initialise on next valid reading
		else:
			self._last_energy = None  # Will re-initialise on next valid reading
